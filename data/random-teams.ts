...
export class RandomTeams extends RandomGen8Teams {
	randomSets: {
		[species: string]: RandomTeamsTypes.RandomSpeciesData;
	} = require("./random-sets.json");
	...
	constructor(format: Format | string, prng: PRNG | PRNGSeed | null) {
		...
		this.moveEnforcementCheckers = {
			Bug: (movePool, moves, abilities, types, counter) =>
				...
				(!counter.get("Bug") &&
					(abilities.has("Tinted Lens") || abilities.has("Adaptability"))),
			...
			Flying: (movePool, moves, abilities, types, counter, species) =>
				!counter.get("Flying") &&
				!["aerodactylmega", "charizardmegay", "mantine"].includes(
					species.id
				) &&
			...
			Ice: (movePool, moves, abilities, types, counter) =>
				!counter.get("Ice") ||
				(!moves.has("blizzard") && movePool.includes("freezedry")) ||
				(abilities.has("Refrigerate") &&
					(movePool.includes("return") ||
						movePool.includes("hypervoice"))),
			...
			Rock: (movePool, moves, abilities, types, counter, species) =>
				!counter.get("Rock") &&
				(species.baseStats.atk >= 100 || abilities.has("Rock Head")),
	...
	newQueryMoves(
		...
		for (const moveid of moves) {
			...
			if (
				...
				(move.basePower &&
					move.basePower <= 60 &&
					!["nuzzle", "rapidspin"].includes(moveid))
			) {
			...
			if (move.basePower || move.basePowerCallback) {
				if (
					...
					(this.priorityPokemon.includes(species.id) && move.priority > 0)
				) {
	...
	addMove(
		...
		const counter = this.newQueryMoves(
			moves,
			species,
			preferredType,
			abilities
	...
	randomMoveset(
		...
		let counter = this.newQueryMoves(
			moves,
			species,
			preferredType,
			abilities
		...
		// Enforce recovery
		if (
			["Bulky Support", "Bulky Attacker", "Bulky Setup", "Staller"].includes(
				role
			)
		) {
		...
		if (role.includes("Setup") || role === "Z-Move user") {
			...
			const setupMoves = movePool.filter(
				(moveid) => SETUP.includes(moveid) && moveid !== "flamecharge"
			);
		...
		if (
			!counter.damagingMoves.size &&
			!(moves.has("uturn") && types.includes("Bug"))
		) {
	...
	shouldCullAbility(
		...
		switch (ability) {
			...
			case "Pixilate":
			case "Refrigerate":
				return ["doubleedge", "hypervoice", "return"].every(
					(m) => !moves.has(m)
				);
			case "Chlorophyll":
			...
			case "Swift Swim":
				return (
					...
					(!moves.has("raindance") &&
						["Rock Head", "Water Absorb"].some((abil) =>
							abilities.has(abil)
						))
				);
			case "Intimidate":
			...
			case "Lightning Rod":
				return (
					types.has("Ground") ||
					species.id === "marowakalola" ||
					((!!teamDetails.rain || moves.has("raindance")) &&
						species.id === "seaking")
				);
			case "Magic Guard":
			case "Speed Boost":
			...
			case "Mold Breaker":
				return (
					species.baseSpecies === "Basculin" ||
					species.id === "pangoro" ||
					species.id === "pinsirmega" ||
			...
			case "Prankster":
				return (
					!counter.get("Status") ||
					(species.id === "tornadus" && moves.has("bulkup"))
				);
			case "Overcoat":
			...
			case "Solar Power":
				return (
					!counter.get("Special") || !teamDetails.sun || !!species.isMega
				);
			case "Sturdy":
				return (
			...
			case "Swarm":
				return (
					(!counter.get("Bug") && !moves.has("uturn")) || !!species.isMega
				);
			case "Technician":
				return (
					...
					moves.has("tailslap") ||
					!!species.isMega ||
					species.id === "persianalola"
				);
			case "Tinted Lens":
				return (
					["illumise", "sigilyph", "yanmega"].some(
						(m) => species.id === m
					) && role !== "Wallbreaker"
				);
			case "Torrent":
			...
			case "Unburden":
				return (
					!!species.isMega ||
					(!counter.get("setup") && !moves.has("acrobatics"))
				);
			case "Water Absorb":
				return (
					moves.has("raindance") ||
					["Drizzle", "Unaware", "Volt Absorb"].some((abil) =>
						abilities.has(abil)
					)
				);
		}
	...
	getAbility(
		...
		if (species.battleOnly && !species.requiredAbility) {
			abilities = new Set(
				Object.values(
					this.dex.species.get(species.battleOnly as string).abilities
				)
			);
		}
		...
		if (
			...
			(abilities.has("Guts") &&
				...
				(moves.has("facade") ||
					(moves.has("sleeptalk") && moves.has("rest"))))
		)
			return "Guts";
		...
		if (
			abilities.has("Gluttony") &&
			(moves.has("recycle") || moves.has("bellydrum"))
		)
			return "Gluttony";
		if (
			abilities.has("Harvest") &&
			(role === "Bulky Support" || role === "Staller")
		)
			return "Harvest";
		if (
			abilities.has("Moxie") &&
			(counter.get("Physical") > 3 || moves.has("bounce"))
		)
			return "Moxie";
		if (abilities.has("Regenerator") && role === "AV Pivot")
			return "Regenerator";
		if (
			abilities.has("Shed Skin") &&
			moves.has("rest") &&
			!moves.has("sleeptalk")
		)
			return "Shed Skin";
		...
		if (
			abilities.has("Unburden") &&
			["acrobatics", "bellydrum", "closecombat"].some((m) => moves.has(m))
		)
			return "Unburden";
		if (
			abilities.has("Weak Armor") &&
			types.has("Water") &&
			counter.get("setup")
		)
			return "Weak Armor";
	...
	getPriorityItem(
		...
		if (role === "Z-Move user") {
			...
			if (preferredType)
				return this.dex.species.get(`Arceus-${preferredType}`)
					.requiredItems![1];
		}
		if (
			!isLead &&
		...
		if (
			ability === "Harvest" ||
			ability === "Cheek Pouch" ||
			(ability === "Emergency Exit" && !!counter.get("Status"))
		)
			return "Sitrus Berry";
		...
		if (species.nfe)
			return species.name === "Scyther" && role === "Fast Attacker"
				? "Choice Band"
				: "Eviolite";
		if (
			["healingwish", "memento", "switcheroo", "trick"].some((m) =>
				moves.has(m)
			)
		) {
			if (
		...
		if (
			moves.has("rest") &&
			!moves.has("sleeptalk") &&
			!["Hydration", "Natural Cure", "Shed Skin"].includes(ability)
		) {
			return "Chesto Berry";
		}
	...
	getItem(
		...
		if (
			...
			species.id !== "jirachi" &&
			["dragontail", "fakeout", "flamecharge", "nuzzle", "rapidspin"].every(
				(m) => !moves.has(m)
			)
		) {
			return scarfReqs &&
		...
		if (
			!counter.get("Status") &&
			(["Fast Support", "Bulky Support", "Bulky Attacker"].some(
				(m) => role === m
			) ||
				moves.has("rapidspin"))
		) {
		...
		if (
			...
			return "Rocky Helmet";
		if (
			["kingsshield", "protect", "spikyshield", "substitute"].some((m) =>
				moves.has(m)
			)
		)
			return "Leftovers";
		if (
			...
			ability !== "Levitate" &&
			species.id !== "golemalola"
		) {
			return "Air Balloon";
		}
		if (
			(role === "Fast Support" || moves.has("stickyweb")) &&
			isLead &&
		...
		if (!counter.get("Status")) {
			return (moves.has("uturn") || moves.has("voltswitch")) &&
				!counter.get("Dragon") &&
				!counter.get("Normal")
				? "Expert Belt"
				: "Life Orb";
		...
		if (
			["Fast Attacker", "Setup Sweeper", "Wallbreaker"].some(
				(m) => role === m
			) &&
			(this.dex.getEffectiveness("Rock", species) < 2 ||
				species.id === "ninjask") &&
	...
	randomSet(
		...
		if (hasHiddenPower && level < 100) {
			...
			if (!hpType)
				throw new Error(
					`hasHiddenPower is true, but no Hidden Power move was found.`
				);
			const HPivs =
				ivs.atk === 0
					? ZeroAttackHPIVs[hpType]
					: this.dex.types.get(hpType).HPivs;
			let iv: StatID;
			for (iv in HPivs) {
				ivs[iv] = HPivs[iv]!;
			}
		...
		if (forme === "Nihilego") {
			while (evs.spd > 1) {
				const spa = Math.floor(
					(Math.floor(
						2 * species.baseStats.spa + ivs.spa + Math.floor(evs.spa / 4)
					) *
						level) /
						100 +
						5
				...
				const spd = Math.floor(
					(Math.floor(
						2 * species.baseStats.spd + ivs.spd + Math.floor(evs.spd / 4)
					) *
						level) /
						100 +
						5
		...
		if (species.id === "porygonz" && role === "Z-Move user") {
			const firstMove = moves.has("shadowball")
				? "shadowball"
				: "thunderbolt";
		...
		for (const restrict of [true, false]) {
			...
			while (baseSpeciesPool.length && pokemon.length < this.maxTeamSize) {
				...
				for (const poke of pokemonPool) {
					...
					if (
						!hasMega &&
						species.baseSpecies === baseSpecies &&
						species.isMega
					)
						canMega = true;
				}
				...
				if (this.gen) {
					...
					if (
						...
						this.randomSets[species.id]["sets"][0]["role"] ===
							"Z-Move user" &&
						teamDetails.zMove
					)
						continue;
				...
				if (set.ability === "Snow Warning" || set.moves.includes("hail"))
					teamDetails.hail = 1;
				if (
					set.moves.includes("raindance") ||
					(set.ability === "Drizzle" && !item.onPrimal)
				)
					teamDetails.rain = 1;
				if (set.ability === "Sand Stream") teamDetails.sand = 1;
				if (
					set.moves.includes("sunnyday") ||
					(set.ability === "Drought" && !item.onPrimal)
				)
					teamDetails.sun = 1;
				...
				if (
					...
					(set.moves.includes("reflect") &&
						set.moves.includes("lightscreen"))
				) {
	...
	} = require("./mods/gen7/factory-sets.json");
	...
	randomFactorySet(
		...
		for (const curSet of setList) {
			...
			for (const abilityString of curSet.ability) {
				...
				if (
					weatherAbilitiesRequire[ability.id] &&
					teamData.weather !== weatherAbilitiesRequire[ability.id]
				)
					continue;
			...
			const fullSetSpec = {
				set: curSet,
				moveVariants: curSetVariants,
				item: curSetItem,
				ability: curSetAbility,
			};
		...
		for (const [i, moveSlot] of setData.set.moves.entries()) {
			moves.push(
				setData.moveVariants
					? moveSlot[setData.moveVariants[i]]
					: this.sample(moveSlot)
			);
		}

		const item = setData.item || this.sampleIfArray(setData.set.item);
		const ability =
			setData.ability || this.sampleIfArray(setData.set.ability);
		const nature = this.sampleIfArray(setData.set.nature);
		const level =
			this.adjustLevel || setData.set.level || (tier === "LC" ? 5 : 100);
		...
		return {
			...
			gender:
				setData.set.gender ||
				species.gender ||
				(this.randomChance(1, 2) ? "M" : "F"),
			item: item || "",
			ability: ability || species.abilities["0"],
			shiny:
				typeof setData.set.shiny === "undefined"
					? this.randomChance(1, 1024)
					: setData.set.shiny,
			level,
			happiness:
				typeof setData.set.happiness === "undefined"
					? 255
					: setData.set.happiness,
			evs: {
				hp: 0,
				atk: 0,
				def: 0,
	...
	randomFactoryTeam(
		...
		const isMonotype =
			!!this.forceMonotype ||
			this.dex.formats.getRuleTable(this.format).has("sametypeclause");
		...
		if (!this.factoryTier) {
			this.factoryTier = isMonotype
				? "Mono"
				: this.sample(["Uber", "OU", "UU", "BU", "RU", "NU", "PU", "ZU", "LC"]);
		} else if (isMonotype && this.factoryTier !== "Mono") {
			// I don't think this can ever happen?
			throw new Error(
				`Can't generate a Monotype Battle Factory set in a battle with factory tier ${this.factoryTier}`
			);
		}
		...
		while (pokemonPool.length && pokemon.length < this.maxTeamSize) {
			const species = this.dex.species.get(
				this.sampleNoReplace(pokemonPool)
			);
			if (!species.exists) continue;

			// Lessen the need of deleting sets of Pokemon after tier shifts
			if (
				this.factoryTier in tierValues &&
				species.tier in tierValues &&
				tierValues[species.tier] > tierValues[this.factoryTier]
			)
				continue;

			const speciesFlags =
				this.randomFactorySets[this.factoryTier][species.id].flags;
			...
			if (isMonotype) {
				...
				for (const typeName of types) {
					if (
						teamData.typeCount[typeName] >= 2 * limitFactor &&
						this.randomChance(4, 5)
					) {
						skip = true;
						break;
					}
			...
			teamData.typeComboCount[typeCombo] =
				teamData.typeComboCount[typeCombo] + 1 || 1;
			...
			for (const typeName of this.dex.types.names()) {
				...
				if (
					resistanceAbilities[abilityState.id]?.includes(typeName) ||
					!this.dex.getImmunity(typeName, types)
				) {
					// Heuristic: assume that Pokémon with these abilities don't have (too) negative typing.
					teamData.resistances[typeName] =
						(teamData.resistances[typeName] || 0) + 1;
				...
				if (typeMod < 0) {
					...
					teamData.weaknesses[typeName] =
						(teamData.weaknesses[typeName] || 0) + 1;
				}
			}
		}
	...
	randomBSSFactorySets: AnyObject = require("./mods/gen7/bss-factory-sets.json");
	...
	randomBSSFactoryTeam(
		...
		while (pokemonPool.length && pokemon.length < this.maxTeamSize) {
			...
			if (
				teamData.megaCount + (teamData.zCount ? teamData.zCount : 0) >= 3 &&
				speciesFlags.megaOnly
			)
				continue;
			...
			for (const type of types) {
				if (
					teamData.typeCount[type] >= 2 * limitFactor &&
					this.randomChance(4, 5)
				) {
					skip = true;
					break;
			...
			if (
				teamData.eeveeLimCount &&
				teamData.eeveeLimCount >= 1 &&
				speciesFlags.limEevee
			)
				continue;
