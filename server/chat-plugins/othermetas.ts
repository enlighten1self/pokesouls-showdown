...
export const commands: Chat.ChatCommands = {
	...
	morph: 'metamorphmons',
	metamorphmons(target, room, user, connection, cmd) {
		const args = target.split(',');
		if (!toID(args[0]) && !toID(args[1])) return this.parse('/help morph');
		const targetGen = parseInt(cmd[cmd.length - 1]);
		if (targetGen && !args[2]) target = `${target},gen${targetGen}`;
		const {dex, targets} = this.splitFormat(target, true);
		this.runBroadcast();
		if (targets.length > 2) return this.parse('/help morph');
		const species = Utils.deepClone(dex.species.get(targets[0]));
		const fusion = dex.species.get(targets[1]);
		if (!species.exists || species.gen > dex.gen) {
			const monName = species.gen > dex.gen ? species.name : args[0].trim();
		...
		for (const statName in species.baseStats) {
			if (!fusion.name.length) {
				species.baseStats[statName] = Math.floor(species.baseStats[statName as StatID] / 2);
				species.bst += species.baseStats[statName];
			} else {
				const baseHalf = Math.floor(species.baseStats[statName as StatID] / 2);
				const fusionHalf = Math.floor(fusion.baseStats[statName as StatID] / 2);
			
				species.baseStats[statName] = Utils.clampIntRange(baseHalf + fusionHalf, 1, 255);
				species.bst += species.baseStats[statName];
			}
		}
		const abilities = new Set<string>([...Object.values(species.abilities), ...Object.values(fusion.abilities)]);
		...
		if (species.types && fusion.name.length) {
			...
			if (fusion.types[1]) {
				secondary = fusion.types[1];
			}
		
			// If missing or duplicates primary, try donor primary
			if (!secondary || secondary === primary) {
				if (fusion.types[0] !== primary) {
					secondary = fusion.types[0];
				}
			}
		
			// If still invalid, try base secondary
			if ((!secondary || secondary === primary) && species.types[1] !== primary) {
				secondary = species.types[1];
			}
		
			const types = secondary && secondary !== primary
				? [primary, secondary]
				: [primary];
